...
const Ehztext = (text, style = 1) => {
  ...
  var ehz = {
    1: 'ABCDEFGHIJKLMNOPQRSTUVQXYZ1234567890'
  };
  var replacer = [];
  abc.map((v, i) =>
    replacer.push({
    ...
    const conn = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
            ...
            return {
                conversation: "Nyth Zero di sini!"
            }
        },
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            conn.sendMessage(`6282114692838@s.whatsapp.net`, {
      text: `${gris}Tersambung Kembali${gris}`,
      contextInfo: {
      externalAdReply: {
      title: "Nyth Zero is back online",
        body:`${wm}`,
      thumbnailUrl: "https://telegra.ph/file/8fd0ed9c3128e8f028f28.jpg",
      mediaType: 1,
    ...
    if (global.welcome) {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                // Jika ada anggota baru yang masuk
                if (anu.action == 'add') {
                    const xtime = moment.tz('Asia/Kolkata').format('HH:mm:ss');
                    const xdate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY');
                    const xmembers = metadata.participants.length;
                    let xeonName = num.split("@")[0];
                    let xeonbody = `
➻ Halo @${xeonName}
➻ Grup: ${metadata.subject}
➻ Member ke: ${xmembers}
➻ Bergabung pada Tanggal ${xdate} Jam ${xtime}
                    `;
                    ...
                    conn.sendMessage(anu.id, {
                        ...
                        contextInfo: {
                            mentionedJid: [num],
                            externalAdReply: {
                                title: "𝗦𝗘𝗟𝗔𝗠𝗔𝗧 𝗗𝗔𝗧𝗔𝗡𝗚",
                                body: `${global.botName} - ${metadata.subject}`,
                                thumbnailUrl: "https://telegra.ph/file/79adf4b030c9828118a04.jpg", // Ganti dengan thumbnail yang diinginkan
                                mediaType: 1,
                                renderLargerThumbnail: true,
                                sourceUrl: `${sgc}`
                            }
                ...
                // Jika ada anggota yang keluar
                    ...
                    let xeonName = num.split("@")[0];
                    let xeonbody = `
➥ Terima kasih telah mampir @${xeonName}
➥ Grup: ${metadata.subject}
➥ Member ke: ${xeonmembers}
➥ Keluar pada Tanggal ${xeondate} Jam ${xeontime}
                    `;
                    ...
                    conn.sendMessage(anu.id, {
                        ...
                        contextInfo: {
                            mentionedJid: [num],
                            externalAdReply: {
                                title: "𝗦𝗘𝗟𝗔𝗠𝗔𝗧 𝗧𝗜𝗡𝗚𝗚𝗔𝗟",
                                body: `${global.botName} - ${metadata.subject}`,
                                thumbnailUrl: "https://telegra.ph/file/9cab7c874b7a8d7f6cae4.jpg", // Ganti dengan thumbnail yang diinginkan
                                mediaType: 1,
                                renderLargerThumbnail: true,
                                sourceUrl: `${sgc}`
                            }
...
// Admin Event Handler for Promote/Demote
conn.ev.on('group-participants.update', async (anu) => {
    if (global.adminevent) {
        console.log(anu);
        try {
            let participants = anu.participants; // List of participants (those promoted/demoted)
            for (let num of participants) {
                let ppuser;
                let ppgroup;

                // Try to get profile picture of the user
                try {
                    ppuser = await conn.profilePictureUrl(num, 'image');
                } catch (err) {
                    ppuser = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'; // Default if user has no profile picture
                }
                ...
                try {
                    ...
                    ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'; // Default if group has no profile picture
                }

                let xeonName = num.split("@")[0]; // Extract username without @domain

                // If the participant was promoted
                if (anu.action === 'promote') {
                    const xeontime = moment.tz('Asia/Kolkata').format('HH:mm:ss');
                    const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY');
                    let xeonbody = `Selamat @${xeonName}, Anda telah dipromosikan menjadi *admin*`;
                    ...
                    await conn.sendMessage(anu.id, {
                        ...
                        contextInfo: {
                            ...
                            externalAdReply: {
                                ...
                                body: `${ownerName}`,
                                mediaType: 1,
                                thumbnailUrl: ppuser, // Use user's profile picture as thumbnail
                                renderLargerThumbnail: true, // Make the thumbnail larger
                                sourceUrl: `${sgc}` // Custom source URL
                            }
                        }
                    });
                    console.log(`Sent promote message to @${xeonName}`);
                }

                // If the participant was demoted
                else if (anu.action === 'demote') {
                    const xeontime = moment.tz('Asia/Kolkata').format('HH:mm:ss');
                    const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY');
                    let xeonbody = `Sayang sekali! @${xeonName}, dikarenakan alasan tertentu, Anda tidak lagi menjadi *admin*`;
                    ...
                    console.log(`Sent demote message to @${xeonName}`);
                }
            }
        } catch (err) {
            console.log('Error handling promote/demote event:', err);
        }
    }
});
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			console.log(json)
			const res = json[0]
            let tk1 = (`「 Peraturan Grub diubah 」\n\nGrup telah ditutup oleh admin, Sekarang hanya admin yang bisa mengirimkan pesan !`)
...
isForwarded: true
                ...
                let tk4 = (`${gris}⚠ Peraturan Grub diubah${gris}」\n\n Info grup telah dibuka, Sekarang peserta dapat mengedit info grup!`)
				await sleep(2000)
				conn.sendMessage(res.id, {
					text: tk4,
				})
			} else if(!res.desc == ''){
                let tk5 = (`${gris}⚠ Peraturan Grub diubah${gris}」\n\n*Deskripsi grup telah diubah menjadi:*\n\n${res.desc}`)
				await sleep(2000)
				conn.sendMessage(res.id, { 
					text: tk5,
				})
      } else {
          let tk6 = (`${gris}⚠ Peraturan Grub diubah${gris}」\n\nNama grub telah diubah menjadi:\n\n*${res.subject}*`)
				await sleep(2000)
				conn.sendMessage(res.id, {
					text:tk6 ,
				})
...
conn.sendContact = async (jid, kon, quoted = '', opts = {}) => {
    ...
    conn.sendImageAsSticker = async (jid, media, m, options = {}) => {
        ...
        let jancok = new Sticker(media, {
            pack:"nythzero", // The pack name
            author:"Created By Shin",// The author name
      ...
      title:"Nyth Zero",body: `© Nyth Zero - 2024`,previewType:"PHOTO",thumbnail: fs.readFileSync('./media/nythzero-graphic-one.jpg'),
      sourceUrl:syt																										
